...
trigger pattern list and the reminder body so the Codex, Claude Code, and any
other host wrappers can never drift apart on substance.

Hook protocol (Codex): read JSON event from stdin, write JSON to stdout with
`additionalContext` if we want to inject text. Exit 0 even on no-op; exit
